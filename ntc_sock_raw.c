...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		// only process Internet Protocol packet
		if (mac_hdr->h_proto != ETH_P_IP && mac_hdr->h_proto != ETH_P_IPV6) {
			printf("%X %lx\n", (u_char) mac_hdr->h_proto, ETH_P_IP);
			continue;
		}

		ip_hdr = buffer + sizeof(struct eth_hdr);

		printf("截获内容长度 %d\n", len);

		printf("源 MAC:%X:%X:%X:%X:%X:%X %d",
			   ...
			   (u_char) mac_hdr->h_proto
		);

		printf(" ==> 目的 MAC:%X:%X:%X:%X:%X:%X %d\n",
